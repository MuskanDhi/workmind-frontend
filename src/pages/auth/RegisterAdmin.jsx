...
const RegisterAdmin = () => {
  const [formData, setFormData] = useState({
    adminName: '', adminEmail: '', password: '', companyName: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/api/company/register', formData);
      ...
      console.error("Registration Error Stack:", stack);
      toast.error(serverMsg ? `${serverMsg}` : `Failed to register: ${error.message}`);
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div style={{
            ...
            <div className="fade-in">
              <h2 style={{ fontSize:20, fontWeight:700, color:'#111827', marginBottom:4 }}>Admin Registration</h2>
              <p style={{ fontSize:13, color:'#6B7280', marginBottom:24 }}>Create your company workspace</p>
          ...
          <p style={{ textAlign:'center', marginTop:20, color:'rgba(255,255,255,0.25)', fontSize:12 }}>
            Registering creates a new company workspace
          </p>
        </div>
      </div>
    </>
